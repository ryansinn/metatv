...
# ---------------------------------------------------------------------------
# 5. A dislike suppresses siblings sharing content_key
# ---------------------------------------------------------------------------

def test_dislike_suppresses_content_key_siblings(tmp_path):
    """Disliking one variant must suppress every content_key sibling, even when
    the generic engaged/fingerprint collapse is bypassed via dedupe_overrides
    (e.g. an "Other Versions" caller) — isolating the disliked_ids widening
    itself as the mechanism doing the suppressing, rather than the pre-existing
    generic same-production collapse that would otherwise also catch this case.

    The disliked title's own genre ("Thriller") is given TWO independent liked
    signals so its net weight stays positive even after the -1 dislike is
    netted in — this stops the unrelated "total <= 0" score-skip check from
    accidentally suppressing the sibling candidate for the wrong reason, so
    only the disliked_ids exclusion path can be responsible for the outcome
    this test asserts.
    """
    from metatv.core.preference_engine import compute_weights, score_candidates

    db = _make_db(tmp_path / "dislike.db")
    with db.session_scope() as session:
        # Positive "Action" signal used to score the unrelated "good" title.
        signal_meta = _make_metadata(session, "Signal", genres=["Action"], year=2020)
        signal = _make_channel(
            session, name="EN Liked Signal", content_key="signal|movie|2020",
            detected_prefix="EN", metadata_id=signal_meta.id,
            last_played=datetime(2024, 1, 1),
        )
        _make_rating(session, signal.id, 1)

        # Two independent positive "Thriller" signals so that genre stays net
        # positive even after the -1 dislike below is netted into it — see
        # docstring above.
        for j in range(2):
            thriller_meta = _make_metadata(session, f"Thriller Signal {j}",
                                           genres=["Thriller"], year=2015 + j)
            thriller_ch = _make_channel(
                session, name=f"EN Thriller Signal {j}",
                content_key=f"thriller-signal-{j}|movie|{2015 + j}",
                detected_prefix="EN", metadata_id=thriller_meta.id,
                last_played=datetime(2024, 1, 1),
            )
            _make_rating(session, thriller_ch.id, 1)

        # A disliked title (genre Thriller — net weight stays positive) with
        # two content_key siblings.
        bad_meta = _make_metadata(session, "Bad Movie", genres=["Thriller"], year=2019)
        ch_a = _make_channel(
            session, cid="bad-a", name="EN Bad Movie", content_key="bad|movie|2019",
            detected_prefix="EN", metadata_id=bad_meta.id,
        )
        ch_b = _make_channel(
            session, cid="bad-b", name="FR Bad Movie", content_key="bad|movie|2019",
            detected_prefix="FR", metadata_id=bad_meta.id,
        )
        _make_rating(session, ch_a.id, -1)

        # An unrelated title that must still surface.
        good_meta = _make_metadata(session, "Good Movie", genres=["Action"], year=2021)
        _make_channel(
            session, cid="good", name="EN Good Movie", content_key="good|movie|2021",
            detected_prefix="EN", metadata_id=good_meta.id,
        )

    with db.session_scope(commit=False) as session:
        weights = compute_weights(session)
        assert weights.genres.get("Thriller", 0.0) > 0.0, (
            "test setup sanity check: Thriller must stay net-positive after "
            "netting in the -1 dislike, so 'bad-b' can only be excluded via "
            "the disliked_ids path, never the unrelated total<=0 score-skip"
        )
        results = score_candidates(
            session, weights, limit=30,
            dedupe_overrides={"bad-a", "bad-b"},
        )

    ids = {r.channel_id for r in results}
    assert "bad-a" not in ids and "bad-b" not in ids, (
        f"both the disliked channel and its content_key sibling must be "
        f"suppressed even when dedupe_overrides bypasses the generic "
        f"same-production collapse; got {ids}"
    )
    assert "good" in ids, f"an unrelated title must still surface; got {ids}"
    db.close()
