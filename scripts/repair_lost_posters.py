...
def main() -> int:
    ...
    # ── Route 1: the provider chain (free — reads stored raw_data) ─────────
    ...
    async def _run():
        ...
        for index, (cid, name, title) in enumerate(damaged, 1):
            ...
            try:
                ...
                unresolved.append((cid, label))
                continue
            if getattr(result, args.field, None) if result else None:
                restored += 1
                print(f"  [{index}/{len(damaged)}] {label}: restored from raw_data")
            else:
                unresolved.append((cid, label))

    asyncio.run(_run())
    print(f"\nRoute 1 (stored raw_data): restored {restored} of {len(damaged)}.")

    # ── Route 2: the per-title detail endpoint, for what route 1 missed ────
    if unresolved:
        print(f"\nRoute 2: fetching the detail blob for {len(unresolved)} title(s)…")
        from metatv.core.tmdb_enrichment_manager import TmdbEnrichmentManager

        enricher = TmdbEnrichmentManager(db, config)
        try:
            totals = enricher.harvest_for_channels([cid for cid, _ in unresolved])
        finally:
            enricher.shutdown()
        print(f"  attempted {totals['attempted']}, fetched {totals['fetched']}, "
              f"errors {totals['errors']}")

        remaining = _damaged(db, args.field)
        remaining_ids = {cid for cid, _n, _t in remaining}
        recovered = [label for cid, label in unresolved if cid not in remaining_ids]
        restored += len(recovered)
        for label in recovered[:15]:
            print(f"  · {label}: restored from the detail blob")
        still = [label for cid, label in unresolved if cid in remaining_ids]
    else:
        still = []

    print(f"\nRestored {restored} of {len(damaged)}.")
    if still:
        print(f"{len(still)} still missing — the provider genuinely has no artwork "
              f"for these, which #438 now records without erasing anything:")
        for label in still[:15]:
            print(f"  · {label}")
        if len(still) > 15:
            print(f"  … and {len(still) - 15} more")
    return 0
