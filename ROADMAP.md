...
- [ ] **Name-based adult detection → adult-mode filter** — the adult-mode hide/only filter keys off the provider's `is_adult` API flag only. Channels named with adult prefixes (XXX / ADULT / X — now grouped under the "Adult" prefix-filter category) are NOT caught when the provider doesn't flag them. Optionally detect adult content from the prefix/name at ingestion (set `is_adult`) so adult-mode hides them too. *Display grouping already exists (the "Adult" prefix group); this is the filtering half.*
