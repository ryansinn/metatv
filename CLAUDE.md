...
**Live theme switching is known-broken — don't claim otherwise.** Qt caches the *rendered* stylesheet string, so a widget styled at construction keeps the old palette; there are **838 `setStyleSheet` sites vs 22 `refresh_theme()` methods**, and the gap is not closable by adding sweep entries (#253/#261 tried). Cold launch is correct. Until the style registry lands, don't describe a theme change as applying live, and don't "fix" a reported theme bug by adding one more `refresh_theme()`. Detail: docs/UI_UX_GUIDELINES.md → "Known-broken: live theme switching".
