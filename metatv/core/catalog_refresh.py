...
app." Two halves shipped together: the (now-retired) Sports view said when
the catalog was stale, and a source can opt into automatic refresh via its
existing "Auto-refresh" schedule.
...
# identical to the live half of a full refresh. So a global auto-rate
# (Settings -> Content) drives a LIVE-ONLY refresh — never the full one —
# through the rules below.
#
# (Two other LIVE-1 triggers — a Sports-banner "Refresh sources" action and
# an opening-Sports/Events hook, with their own ``BANNER_STALE_THRESHOLD``/
# ``LIVE_REFRESH_ON_OPEN_COOLDOWN``/``live_refresh_on_view_open_due`` — were
# deleted in dead-code sweep B along with the retired views that called them.
