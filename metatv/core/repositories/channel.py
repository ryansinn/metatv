    ...
    def update_detected_prefixes(
        ...
        for batch_start in range(0, total, _BATCH):
            ...
            for channel in channels:
                ...
                # genres_from_raw() splits on '/'/',' and canonicalises each segment
                # (cross-language alias collapse + HTML-entity unescape) — the SAME
                # derivation discovery_engine._primary_genre() used to do at every
                # render/query. Storing it here is what lets get_by_genre/get_all_genres
                # stop parsing raw_data at read time. detected_genre = first segment
                # (display); detected_genres = every segment (shelf membership).
