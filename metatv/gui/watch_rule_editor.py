"""The Option B rule row — one editor, embedded by every surface that shows rules.
...
and rendered by both Watch Alerts and the EPG watchlist, so the editor is a
widget rather than a block of code inside one of them.
...
displays whatever counts it is handed. The host owns the write (via
``watchlist.update``) and owns the query — the counts come off a background
read, because "how many programmes does this rule match" scans the guide and
...
from PyQt6.QtWidgets import (
    QCheckBox, QComboBox, QFormLayout, QLabel, QLineEdit, QVBoxLayout, QWidget,
)
...
#: ","'), shortened to what fits a dropdown.
_MODE_LABELS = {
    PHRASE: "Phrase",
    ALL_WORDS: "All words",
    ANY_WORD: "Any word",
...
def split_terms(text: str) -> tuple[str, ...]:
    ...
    Shared with nothing yet on purpose: ``WatchRule.terms`` does the same split
    on the stored string, and this is the editor's half of that contract. Kept
    as a named function so the round trip (edit -> store -> match) is testable
    without a widget.
    """
...
class WatchRuleEditor(QWidget):
    """The expanded body of a watch-rule row.

    Signals:
        rule_changed: Emitted with a new :class:`WatchRule` whenever the user
            changes any control. Never emitted by :meth:`set_rule`, which is a
            programmatic restore — see the signal-blocking note there.
    """
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        #: Kept so a caller (and the layout test) can reach a field's label
        ...
        form.addRow(self._label("Match"), self.mode_combo)
        ...
        self.include_input.setToolTip(
            "Separate terms with commas. How they combine is set by Match above.")
        ...
        self.exclude_input.setToolTip(
            "A programme matching any of these is skipped, even if it matches "
            "the Include terms.")
        ...
        # Checked at construction so an editor with no rule loaded still shows
        # the SETTLED DEFAULT rather than describing itself as "contains
        # anywhere" — which is what a bare QCheckBox would claim.
        self.whole_word_check.setChecked(WatchRule(term="").whole_word)
        self.whole_word_check.setToolTip(
            'Off means "contains, anywhere" — "NFL" would also match '
            '"Inflammation".')
        self.whole_word_check.toggled.connect(self._emit)

        self.description_check = QCheckBox("Also search descriptions")
        self.description_check.setToolTip(
            "Looks in the programme synopsis as well as the title. Finds more, "
            "and more of it will be loose matches.")
        self.description_check.toggled.connect(self._emit)
        ...
        options_layout.addWidget(self.description_check)
        form.addRow(self._label("Options"), options)
    ...
    def set_rule(self, rule: WatchRule) -> None:
        ...
        Every widget is set inside one blockSignals pair rather than each
        guarding itself: setting the mode combo fires its own handler, which
        would read the include field before it had been populated and emit a
        rule with the previous row's terms. The project rule is the same one —
        block, set every widget, unblock — and this is exactly the shape it
        exists for.
        """
        self._rule = rule
        widgets = (self.mode_combo, self.include_input, self.exclude_input,
                   self.whole_word_check, self.description_check)
        for widget in widgets:
            widget.blockSignals(True)
        try:
            index = self.mode_combo.findData(rule.match_mode)
            self.mode_combo.setCurrentIndex(index if index >= 0 else 0)
            self.include_input.setText(rule.term)
            self.exclude_input.setText(", ".join(rule.exclude))
            self.whole_word_check.setChecked(rule.whole_word)
            self.description_check.setChecked(rule.search_description)
        finally:
            for widget in widgets:
    ...
    def rule(self) -> WatchRule:
        ...
        return WatchRule(
            ...
            match_mode=self.mode_combo.currentData() or PHRASE,
            search_description=self.description_check.isChecked(),
    ...
    def set_counts(self, matched: int | None, suppressed: int | None,
        ...
        Called from the host's main-thread slot once its background read
        returns — never computed here.
        """
    ...
    def _refresh_summary(self) -> None:
        parts = ["Whole words only" if self.whole_word_check.isChecked()
        ...
        self.summary_label.setToolTip(
            _MODE_HELP.get(self.mode_combo.currentData() or PHRASE, ""))
...
def attach_rule_editor(host, layout, pattern: str, toggle_btn) -> None:
    ...
    def _toggle(_checked=False, ed=editor, btn=toggle_btn):
        ...
        btn.setText(f"Rule {icons.collapse_icon if showing else icons.expand_icon}")
