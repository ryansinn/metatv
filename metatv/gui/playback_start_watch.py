...
**A drop mid-play is not the film ending, and is never replayed automatically
(PLAY-17, then PLAY-19).** Owner's log, 2026-09-11 04:02-04:05: a movie played
from 04:03:35, the origin closed the connection at 04:04:01, ffmpeg's
reconnects were all refused for 60s, the buffer ran dry, and mpv hit EOF and
exited — with ``--idle=once`` — 157s into a feature-length file. The app read
that exactly like the film ending: no report, window gone. :func:`on_loaded_tick`
tracks the last known ``time-pos`` and ``duration`` for the whole play, even
after it has progressed, and :func:`dropped_mid_stream` compares them: a play
that ends more than :data:`DROP_END_MARGIN_S` short of its own duration did
not finish — it dropped. Owner, 2026-09-13, after the automatic-resume version
of this shipped: *"these new playback features are fucking dumb. if a channel
fails to play, it shouldn't be automatically started again, you're just
making more problems."* The drop is reported once and the player is left
alone; the position is already on record via the watch-capture path
(PLAY-9/RESUME-1), so the details pane's own Resume picks it up from there —
this module's job stops at telling the user.
"""
    ...
    * FROZEN — a numeric ``time-pos`` that never increases: mpv accepted the
    ...
    Args:
        ...
        duration: mpv's ``duration`` property (the file's total length, or
            ...
            the rest of this function is a no-op — so :func:`on_player_gone`
            always has the position and duration as of the last live tick to
            report against, whenever the player later vanishes (PLAY-17).
    """
...
def on_player_gone(host: Any, exit_reason: "str | None" = None) -> bool:
    """The last player window disappeared. Returns True if that was reported.
    ...
    *exit_reason* is mpv's own ``Exiting... (reason)`` line, captured by
    ...
    ``Errors when loading file`` while still OPENING). 2026-09-06 10:11: a
    cold play on a one-connection source did exactly that inside 30 s, and
    the old "it loaded, so it played" rule kept it silent.
    ...
    Checked FIRST, ahead of every branch below (PLAY-17, then PLAY-19): a
    progressed play that dropped mid-stream is neither "the user closed it"
    nor "never started" — it is reported once
    (:func:`_report_dropped_mid_stream`) and the player is left exactly where
    it stopped. Nothing here plays anything again; the saved position is what
    lets the details pane offer Resume (PLAY-9/RESUME-1), a separate path this
    module does not touch.
    """
    if host.__dict__.get("_health_ever_progressed"):
        if exit_reason in STREAM_EXIT_REASONS:
            last_pos = host.__dict__.get("_health_last_time_pos")
            if dropped_mid_stream(last_pos, host.__dict__.get("_health_last_duration")):
                _report_dropped_mid_stream(host, last_pos, exit_reason)
                return True
        return False               # it played, then the user closed it (or genuinely finished)
...
def _report_dropped_mid_stream(host: Any, last_pos: float, exit_reason: "str | None") -> None:
    """A progressed play whose stream dropped mid-play: report it, once, and
    leave the player exactly where it stopped.

    Owner, 2026-09-13: *"if a channel fails to play, it shouldn't be
    automatically started again, you're just making more problems."* This
    used to replay from a few seconds before the cut, up to three times, before
    telling the user (PLAY-17). Now it only tells the user — the position is
    already on record via the watch-capture path (PLAY-9/RESUME-1), so the
    details pane's own Resume picks it up from here.
    """
    att = host.__dict__.get("_health_attempt")
    name = att.channel_name if att else "that channel"
    pos = max(0, int(last_pos) - 5)
    m, ss = pos // 60, f"{pos % 60:02d}"
    logger.warning("stream dropped mid-play for {!r} at {}s of {}s ({})", name,
                   last_pos, host.__dict__.get("_health_last_duration"), exit_reason)
    try:
        host.status(
            f"{name}: the stream dropped at {m}:{ss} — Resume picks up from there",
            ms=0, level="warn")
    except Exception:                                    # pragma: no cover
        logger.exception("could not update the status bar")
    try:
        host.notification_manager.show(
            ...
            message=(f"{name} dropped at {m}:{ss}. Resume is available from "
                     "there whenever you're ready — nothing is replayed "
                     "automatically."),
            type="warning",
            auto_dismiss_ms=8000,
        )
    except Exception:                                    # pragma: no cover
        logger.exception("could not show the drop notification")
