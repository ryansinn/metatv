...
| 30 | Search results — sections, sub-headings, band, `All\|Word`, sub-filter | `search_ranking.py`, `channel_list_grouping.py`, `channel_list_section_band.py`, `channel_list_filtering.py` | ✅ #663–#671 — built to **Concept E** in the "Finding Tron" artifact, which is the settled design for THIS surface. The first build used the sidebar's `GroupHeading` grammar instead and came out inverted: the search band's **label is bright and its count muted** (the sidebar is the other way round, because there the label is the constant and here it names the field that MATCHED), it keeps a caret at the FAR RIGHT, and a `flex:1` hairline runs label→count — that rule's absence is what read as "empty space on the right". `All` ⊇ `Word` (tiers 0-3 vs 0-2), which is why there is no third state. |
