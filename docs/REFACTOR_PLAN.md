...
| D7 | `sidebar/base.py` over the 1000-line cap (was F11) | `metatv/gui/sidebar/base.py` | 1319 lines. The pressure pass — `PressureGroup`, `pressure_groups`, `_apply_pressure`, `_content_height`, `_schedule_pressure`, `resizeEvent` — moved to `sidebar/section_pressure.py` as `SectionPressureMixin`, mixed in beside `RowBudgetMixin`, which is the same shape: a cross-cutting behaviour reaching the section's widgets through `self` and owning none of them. base.py is now ~1210. **Still over the cap** — the header builders and the splitter arithmetic (`_grow_in_splitter` / `_release_in_splitter` / `_floor_of`) are the next two cohesive units, and neither is about what a SECTION is. Watch the MRO when moving a Qt override: `resizeEvent` now resolves to `RowBudgetMixin`'s, which reaches the pressure one only because it calls `super()`. |
