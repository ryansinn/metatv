...
| F11 | **`sidebar/base.py` is 1319 lines against the 1000 cap** | `metatv/gui/sidebar/base.py` | Already over before this session (1113 at #479, no baseline entry so it takes the flat cap for new files); #482 took it to 1319 by adding the pressure machinery and #483 added `GroupHeading.set_news`. The clean first extraction is the pressure pass — `PressureGroup`, `pressure_groups()`, `_apply_pressure`, `_content_height`, `_schedule_pressure`, ~130 lines — into a `sidebar/section_pressure.py` mixin, which is exactly the shape `row_budget.py`'s `RowBudgetMixin` already has in this same class. That alone does not reach 1000, so the rest wants a look at the same time: the header builders and the splitter-arithmetic helpers (`_grow_in_splitter` / `_release_in_splitter` / `_floor_of`) are each cohesive and neither is about what a SECTION is. |
