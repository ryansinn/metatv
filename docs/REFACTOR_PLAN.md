...
| D17 | Transient-lock retry exists once and a fourth caller cannot reach it | `ChannelRepository._retry_on_lock` (`repositories/channel.py:1593`) is a bound METHOD, shared by three `update_detected_prefixes` phases — its own docstring records that it was created because "only the batch commit had retry coverage" when the tmdb-sibling phase crashed on `database is locked` (owner log 2026-08-01). The provider store's `_flush_batch` (`provider_loader.py`, a `@staticmethod` taking `(session, batch)`) is the statement that ACTUALLY died on 2026-08-31 and has no retry, because it cannot see a method bound to `self.session` | **OPEN — not a duplicate yet, a chokepoint that stops one caller short.** #601 removed the 69.3s hold that caused the failure, so this is defence in depth, not a live bug. Shape: lift `_retry_on_lock` to a module-level `retry_on_lock(session, label, fn, ...)` and have both call it. Safe for `_flush_batch` specifically because its statement is an idempotent `ON CONFLICT DO UPDATE` upsert, which satisfies the helper's documented "`fn` must be safe to re-run from scratch" contract. **Worth doing before downloads/DVR land**, since each adds a writer to a single-writer database |
