...
| D21 | Two paths size the same list, and only one of them handles "empty" | `row_budget.apply_row_budget` (the `sidebar_show_more_row` / "Show N more" mode) vs `_show_all_rows` (the default) | **OPEN, measured.** The default path fits an empty list to 0 as of 2026-09-02; the budget path returns before sizing and keeps the splitter's allocation — 190px for an empty Recordings section, worse than the 108px the default path used to produce. **Not a pure duplicate and the obvious fix is wrong**: fitting the empty list to 0 makes its viewport 0, which is the "not laid out yet" branch, which also returns without sizing, so the first rows arrive into a 0px list; fitting on both branches removes the `+N more` tail altogether (measured: 5 rows, 205px, nothing hidden) because a list pinned to its content can never overflow. The real fix is for the budget to size against the SECTION's height rather than the list's own. Only reachable with the non-default `sidebar_show_more_row` setting on |
