...
| D13 | The view-switcher chip list | `app_header._create_nav_group` built the chips from a local `specs` list; `main_window_nav._deactivate_view_chips` kept its own copy (`[search, epg, prefs, discover]` plus a conditional `recipe`) | Promoted to a module-level `NAV_CHIP_SPECS` that both read. **The failure mode is silent**: a chip missing from the second copy stays LIT while another view is showing — nothing raises, it just looks broken. Adding the sixth chip is what made deriving it cheaper than extending it. `test_the_switcher_and_the_chip_deactivator_read_one_list` fails if the hand-written copy returns |
