...
| R2 | **RESOLVED** — Migration-task boilerplate: 19 identical `__init__`, 12 identical `needs_run`, and FIVE byte-identical `run()` bodies | `core/migrations/category_marker_backfill.py`, `collection_token_cleanup_backfill.py`, `detected_genre_backfill.py`, `detected_title_reparse.py`, `restricted_backfill.py` — each `run()` opened a `session_scope`, built a `RepositoryFactory` and called `update_detected_prefixes(provider_id=None)` verbatim | `core/migrations/base.py` gained `VersionGatedTask` — an OPTIONAL base beside the `MigrationTask` Protocol (which stays a Protocol by design; tasks that don't fit this shape, e.g. `MetadataRescanTask`'s extra constructor arg, keep hand-rolling it) supplying `__init__(db)`/`needs_run(config)`/`on_completed(config)` off two class attrs (`VERSION_FIELD`, `CURRENT_VERSION`). New `core/migrations/detected_fields_reparse.py` `DetectedFieldsReparseBase(VersionGatedTask)` supplies the one shared `run()`. Each of the five modules is now a ~15-line subclass setting `id`/`label`/`VERSION_FIELD`/`CURRENT_VERSION` — still FIVE registered tasks (`main_window.py`'s five `register(...)` calls are untouched, so its ids/labels and the What's New/QA history still name them individually) and still satisfies the `MigrationTask` Protocol by duck typing. **Coalescing rejected, not just skipped**: an upgrade where several of these five are pending still runs five full `update_detected_prefixes()` passes, not one. `MigrationManager._run_all` computes its `pending` list once, up front, and calls each task's `run()`/`on_completed()` independently with no config/session threaded between sibling task instances; making one task's pass satisfy its siblings would need new cross-instance shared state for a cosmetic startup saving, and the manager's contract (each task owns its own completion bookkeeping) was not to be touched by this slice. Tests: `tests/test_migration_base.py` (the shared base classes directly, including that two sibling tasks still run two independent passes) plus the five pre-existing per-task suites, unmodified and still green against the shared base. |
| R3 | **RESOLVED** — `_acquire_slot`/`_release_slot` ×3, identical shape (None-accountant-proceeds, exception-swallowed) | `core/epg_fetch.py`, `core/series_monitor.py`, `core/tmdb_enrichment_manager.py` — only the accountant `kind`/`preempt_kinds`/log-prefix differed | Two module functions on `core/connection_accountant.py`: `acquire_or_proceed(accountant, provider_id, kind, holder_id, preempt_kinds, *, label)` / `release_quietly(accountant, provider_id, holder_id, *, label)`. All three callers' methods are now one-liners; the None-means-proceed and exception-swallowed semantics (background-poll-must-yield territory) are preserved exactly, `label` reproduces each caller's original log prefix (`"epg"`/`"series_monitor"`/`"tmdb_enrich"`). |
| R4 | **RESOLVED** — `_defer_for_migration` ×2, identical polling loop | `core/metadata_enrichment_queue.py`, `core/tmdb_enrichment_manager.py` (the first's own docstring called out the second) | `core/migration_gate.py` gained `wait_until_idle(migration_manager, *, max_wait_s, poll_s, should_stop) -> bool`; both callers' `_defer_for_migration` are now a call plus a one-line log. Existing `tests/test_tmdb_enrichment.py::test_defer_for_migration_*` updated to patch `migration_gate.time.sleep` (the sleep call moved modules); new `tests/test_migration_gate.py` covers `wait_until_idle` directly (no-manager, already-idle, polls-to-idle, bounded by `max_wait_s`, `should_stop` breaks early). |
| R5 | **RESOLVED** — The single-writer async queue (pool/queue/flush/shutdown) duplicated | `core/profile_store.py` and `core/watchlist.py`, ~100 lines each — real policy difference kept: watchlist replays `_pending` over reads and carries a `_write_error_handler`; profile_store is write-only | New `core/queued_writer.py` `class SingleWriter` (`submit`/`flush`/`shutdown`, same timeouts and log lines as before) that both modules COMPOSE. Each keeps its own public API and its own policy on top — watchlist's `_pending: list[_PendingWrite]` replay bookkeeping and `_write_error_handler` are untouched; profile_store stays write-only. `profile_store.py` lost its own `_ensure_writer`/`_forget`/`_lock`; `watchlist.py` lost its own `_writer_pool`. |
| R11 | **RESOLVED** — `_is_superscript_char`/`_is_superscript`, identical 8-line NFKD predicate | `core/channel_name_utils.py` (canonical, per CLAUDE.md "curated lookup data lives only here") vs `core/xmltv_parser.py`'s own copy | `xmltv_parser.py` now imports `_is_superscript_char` from `channel_name_utils` and its own copy (plus the now-unused `import unicodedata`) is deleted. `tests/test_xmltv_superscript_dedup.py` asserts the imported name IS the same object (not just equal behaviour) and pins `_drop_superscript_runs`'s behaviour. |
| R12 | **RESOLVED** — `_parse_stored_year`, identical function | `metadata_providers/omdb.py`, `metadata_providers/tmdb.py` | Moved to `metadata_providers/base.py` as `MetadataProviderPlugin._parse_stored_year` (a `@staticmethod`, since both callers are instance methods on subclasses of that base); both call sites now read `self._parse_stored_year(...)`. `tmdb.py`'s unrelated `_year_from_date` (parses a TMDb API ISO date, a different shape) was left alone. Tests added to `tests/test_external_metadata_providers.py`. |
| D18 | `tests/conftest.py`'s `wire_settings_widgets` umbrella sat unused | Six files hand-listed the nine per-tab `wire_settings_*` factories in a `SettingsDialog.__new__` skeleton: `test_split_streams_ui.py`, `test_resume_default.py`, `test_settings_playback_tab.py`, `test_watch_completion_visibility.py`, `test_refresh_all_skip_inactive.py`, `test_settings_tab_layout.py` (`test_watch_completion_visibility.py` even called `wire_settings_signal_widgets` twice) | Each file's full-tab wiring block replaced with one `wire_settings_widgets(dlg)` call, done by hand per file (a regex broke three files when tried before — see the umbrella's docstring). Per-file widgets the umbrella does NOT build (the test's own extra Playback/Interface controls beyond what a `wire_settings_*` factory covers) were left in place untouched. Three other files (`test_details_double_click_actions.py`, `test_graduated_watch_progress.py`, `test_live_refresh_mode_setting.py`) call one or two of the nine factories directly for a narrower, deliberately partial skeleton — not the umbrella's job, left alone (TESTS-1). |
| D19 | 193 local copies of a file-backed `Database` test fixture (`tests/`) | `def db(`/`file_db`/`tmp_db`/`_make_db`/`_make_file_backed_db`/`_db`, byte-identical modulo fixture name, db filename, docstring, `close()` vs `engine.dispose()` | `conftest.py` gained `make_file_db(path)` (the two-line construct-and-`create_tables()`) plus fixtures `db`/`file_db`/`tmp_db` (`file_db`/`tmp_db` are aliases returning `db`, so no call-site edits were needed in files that already named their local fixture one of those three). 70 canonical fixture defs (39 `db`, 27 `file_db`, 4 `tmp_db`) deleted outright — pytest falls through to the conftest fixture of the same name. 6 bare `_make_db(tmp_path)` in-body helpers (no `@pytest.fixture`, called directly with a filename baked in — `test_alerts_background_refresh.py`, `test_catalog_refresh_tick.py`, `test_metadata_title_cleanup.py`, `test_scoping_provenance.py`, `test_tag_collection_click.py`, `test_tag_provenance_display.py`) had their `def` deleted and call sites rewritten to `make_file_db(tmp_path / "<their own filename>")`. Every deletion re-ran `ruff --fix --select F401` (an unused `Database`/`Path` import, or a now-unused `import pytest` where the deleted fixture was the file's only decorator use). **Not chased, logged here rather than fixed:** (1) **17 files** define a fixture *named* `db` (`@pytest.fixture`, real file-backed `Database`, `create_tables()`) that ends in `return database` — **not** `yield` + `close()`/`dispose()` — so it never runs cleanup at all. That is not "modulo close() vs dispose()", it is a missing teardown step entirely — a policy gap, not a copy of the canonical shape — so converting it to the shared `db` fixture would be a behavior change (adds cleanup that was never there) rather than a pure dedup: `test_epg_delete_chunking.py`, `test_metadata_year_backfill.py`, `test_profile_store.py`, `test_prune_vanished_channels.py`, `test_raw_field_backfill.py`, `test_record_from_the_guide.py`, `test_record_window.py`, `test_recording_manager.py`, `test_saved_recipes_as_shelves.py`, `test_search_sections.py`, `test_signal_check_manager.py`, `test_special_content_queries.py`, `test_sports_lane_ordering.py`, `test_sports_reclassify.py`, `test_stream_retry_survives_locked_db.py`, `test_watchlist_store.py`, `test_watchlist_write_lock_retry.py`. (2) **4 files** carry the exact canonical shape (construct → `create_tables()` → `yield` → `close()`/`dispose()`) under a fixture name outside the `db`/`file_db`/`tmp_db` alias set, so deleting them would need call-site renames across the file rather than a pure deletion — out of this slice's scope ("do not chase every copy"): `test_orphan_cascade_heal.py` (`db_path`), `test_provider_loader_session.py` (`mem_db`), `test_image_cache_async_entry.py` (`profile_db`), `test_watchlist_surfaces_honor_match_mode.py` (`_bound_db`, itself a bare non-fixture helper). (3) Class-scoped copies (`TestX.db`) and `:memory:` copies (`db_session`'s job) were left alone per the brief, by design. |
| D20 | A 32-40-key `load_channels` `params` dict hand-copied in four test files | `test_hidden_accounting_dead_streams.py`, `test_dead_gate_comparison_skipped_when_empty.py`, `test_filter_transparency.py`, `test_get_all_include_raw.py`, each under a local `_params(**overrides)` — every copy's docstring claimed "a full params dict shaped like `load_channels` builds", but none of the four actually carried 8 keys `_ChannelListMixin._query_channels` reads (`bypass_keyword_exclusions`, `collapse_variants`, `dead_signal_streak_floor`, `downloaded_only`, `excluded_keywords`, `excluded_tag_content_types`, `facets_hiding_untagged`, `raw_fetched`) — the gap never surfaced because every read went through `params.get(...)` and silently defaulted to `None`/falsy | `conftest.py` gained `channel_query_params(**overrides)`: base = the union of the four copies' keys with their shared default values, `bypass_dead_gate` (the one key the four copies disagreed on presence of) resolved to production's default (`False`), plus the 8 missing-everywhere keys added with production defaults. All four `_params` defs deleted; call sites renamed to `channel_query_params(...)`. Guard: `tests/test_channel_query_params_helper.py` AST-walks the real `_query_channels` for every `params[...]`/`params.get(...)` read and asserts each key is present in `channel_query_params()` — a new key added to production now fails there once instead of defaulting silently in N copies. |
| UI-8 | **RESOLVED (STATUS-1)** — 95 raw `self.status_bar.showMessage(...)` call sites, no shared shape | 15 files: `main_window_channels.py` (17), `main_window_downloads.py` (16), `main_window_series_playback.py` (11), `main_window_streaming.py` (9), `main_window_favorites.py` (8), `main_window_history.py` (7), `main_window_providers.py` (5), `main_window_series.py` (5), `main_window.py` (4), `playback_start_watch.py` (4), `main_window_menu_actions.py` (3), `main_window_alerts.py` (2), `main_window_nav.py` (2), `downloaded_scope.py` (1), `main_window_metadata.py` (1) — 64 persistent (no timeout), the rest split across 2000/3000/4000/5000/6000ms; an error and a hint read identically and nothing could route an error anywhere else | New `main_window_status.py` `_StatusMixin.status(text, *, ms=4000, level="info")`, mixed into `MainWindow`. All 95 sites migrated 1:1 (parity: each site kept its own duration); 24 reclassified `level="error"` (text matching "Error"/"Could not"/"Couldn't"/"Failed"/exception text) and 14 `level="warn"` (unavailable/retrying/degraded-state text) in the same PR — the other 57 stay `"info"`. "warn"/"error" prefix the matching glyph from `icons.py` (`notification_warning_icon`/`notification_error_icon` — already existed) and log via `logger.warning`/`logger.error`. No toast/notification-manager route exists for "error": `NotificationManager.show()` (`core/notifications.py`) is a generic constructor-style API requiring a `title`, not a single-purpose error method — building one was out of scope for this slice, left for later if a real need shows up. `tests/test_status_bar_has_one_path.py` (AST guard, shrink-only `tests/status_bar_allowlist.json`, seeded empty — the app has exactly one `QStatusBar`) proved RED on the pre-fix tree with all 95 sites named, GREEN after; `tests/test_status_chokepoint.py` covers the method directly against a real `QStatusBar`. |
...
| F3 | **RESOLVED** `_remaining_str` | Defined in `epg_agenda_widget.py:18` | Premise had drifted: it was no longer a second implementation nor imported by `epg_watchlist_mixin` (that mixin already imports `remaining_str` straight from `core/epg_utils.py`, as do `epg_on_now_mixin.py` and `epg_channel_card.py`) — it was a verbatim pass-through (`return _remaining_str_base(stop, now)`) to `epg_utils.remaining_str`, which is not the same function as `relative_time.humanize_remaining` (that one never switches to `"Xh Ym left"` past 60 minutes). Moving it to `relative_time.py` would have planted a second, incompatible `remaining_str`. Deleted the redundant wrapper instead; the one call site now uses the imported `epg_utils.remaining_str` directly, matching its other three callers. |
...
| F16 | **RESOLVED — Chunking an oversized `IN (...)`** | `core/sql_batching.py` (new, the chokepoint) vs `provider_loader.py:401` `_CHUNK = 500` | Found while fixing the Recommended crash: `score_candidates` bound 414,759 ids into one `IN` and SQLite RAISED (`too many SQL variables`) rather than degrading. **Not a pure duplicate — `provider_loader`'s loop is correct**, it just predates the shared helper and states the size itself. The wider problem is the population: **147 `.in_()` call sites**, and nothing marks which of them can grow with the catalogue. Migrating `provider_loader` is mechanical; auditing the other 145 is a real pass and needs a way to tell "bounded by user data" from "bounded by the library". **RESOLVED (dedup slice, 2026-09-06):** `provider_loader.py`'s `_report_recycled_ids` now calls `sql_batching.fetch_in_chunks` (same 500-per-chunk default the private loop already used — no behaviour change) and the private `_CHUNK = 500` loop is deleted. Only this one site was migrated; the other 146 `.in_()` call sites remain unaudited, exactly as this row already said. Guard: `tests/test_stream_id_reuse_logging.py::test_recycled_id_lookup_routes_through_sql_batching` (spies on the real `fetch_in_chunks` with 1,200 ids, past the first chunk boundary). |
...
| F9 | **RESOLVED (stale, 2026-09-06)** — Two `test_theme_live_refresh` tests exercise the REMOVED `refresh_theme()` sweep | `tests/test_theme_live_refresh.py` — `TestFilterPanelRefreshTheme.test_refresh_theme_recurses_into_static_section_rows`, `TestMainWindowRefreshThemeSweepsPromotedWidgets.test_sweeps_promoted_locals_and_filter_panel` | Both tests pass on `main` today (CI runs the file on every PR); the `refresh_theme()` they exercise still exists on `MainWindow` as the promoted-widget sweep behind the registry, so the premise ("REMOVED") had drifted. Both fail by direct execution on `main` as well as on any branch, so they are pre-existing, not a branch regression. They assert the hand-maintained `refresh_theme()` sweep still styles widgets — the mechanism CLAUDE.md records as REPLACED by the `theme.style()` registry precisely because an enumeration could not work. A test for a deliberately removed mechanism is worse than none: it reads as coverage of live theming while guarding nothing. Rewrite against the registry or delete; the owner's call which. Sibling of F7 — both are reds the `--quick` gate never runs. |
| F7 | **RESOLVED (stale, 2026-09-06)** — `test_roles_that_declare_the_same_properties_are_reviewed` fails on `main` | `tests/test_theme_role_duplication.py`, budget `_SHAPE_CLUSTER_BUDGET = 41` | `_SHAPE_CLUSTER_BUDGET` is 43 on `main` and the test passes in CI; the budget was re-measured when the two group-heading roles landed. 42 clusters against a budget measured 2026-08-25, i.e. BEFORE #463 added `SIDEBAR_GROUP_HEADING` and `SIDEBAR_GROUP_HEADING_COUNT`. Verified failing on `main` independently of any branch — the quick gate never ran this file. Reviewed: the cluster is those two plus `SIDEBAR_ROW_NEWS`, three roles with three different colours AND three different sizes that merely share the property NAMES `background; color; font-size; font-weight`. Merging them would be wrong. The budget is shrink-only and says "never raise one to make a new twin pass" — these are not twins, but that is a judgement call the owner should make, so it is logged rather than quietly raised. |
...
| F33 | **RESOLVED — the EPG programme-scoping block was pasted eleven times** | `core/repositories/epg.py` — the `excluded_channel_provider_ids` join at 11 sites, plus the `lang_code` `ilike` at 6 | Found while routing the two watchlist queries through the new matcher (WL-1 slice 1). `_scope_watchlist_query` now exists as the chokepoint and **the two watchlist queries use it**; the other nine are left alone deliberately. **Not a pure duplicate — the JOIN is byte-identical at all eleven, but what surrounds it is not:** only some sites also apply `provider_ids` and `lang_code`, and they do not all apply them in the same order or at the same point relative to aggregation (`query.one()`, `query.scalar()`, `query.first()`, and a `spans` list comprehension all follow different copies). Dropping the helper into a site that does not want all three filters would silently change what that query returns — the exact 'policy difference called a duplicate' failure this ledger exists to prevent. Each of the nine needs reading before adoption, which is a pass of its own, not a line-count fix. **RESOLVED (dedup slice, 2026-09-06):** the concern was narrower than it looked — every one of the eleven sites applies the SAME two filters (channel-side exclusion join + `lang_code` ilike), just with other, unrelated filters (`provider_ids`, `hide_filler`, `search_query`, dismissed/hidden lists) interleaved around them; since SQL `AND` filters commute, merging the two shared ones into a single call changes nothing about the result at any site. New chokepoint `_scope_programmes(query, *, excluded_provider_ids=None, lang_code=None)` lives in `epg_watchlist.py` (imported by `epg.py`, so no circular import); all 9 raw copies in `epg.py` plus `epg_watchlist.py`'s own `get_programs_starting_soon` and `_scope_watchlist_query` (which layers the separate FEED-side `provider_ids` filter on top) now call it — 11 call sites, one definition. Guard: `tests/test_epg_channel_provider_scoping.py::TestProgrammeScopingIsAChokepoint` (AST-based, exactly-one-site, shrink-only). |
| F34 | **RESOLVED — `core/alerts.py` has zero callers and held its own keyword matcher** | `metatv/core/alerts.py` (219 lines: `AlertPattern`, `AlertScanner`) | Re-grepped (`AlertScanner`, `AlertPattern\b`, `core.alerts`/`core/alerts`, `alerts import`) and confirmed still dead: no importer anywhere in `metatv/` or `tests/`, only self-references inside the file. Deleted the module. The Watch Alerts surface already routes keyword matching through `watchlist_matching` (see F3/F2), so nothing was left needing the module's second matcher. One test (`test_watchlist_whole_word_matching.py`) named it in a hand-listed "surfaces" population and was updated to drop it. |
| F35 | **A second path composes the selection tint, bypassing `apply_list_selection`'s palette fix** | `gui/sidebar/base.py` `make_seamless()` (~line 284) reads `_theme.LIST_SELECTION_QSS` directly inside its own `style_fn` lambda, rather than calling `theme.apply_list_selection()` | **FIXED.** `make_seamless()` composed the same `LIST_SELECTION_QSS` rule SEL-1 fixed but never pinned `HighlightedText`/`Highlight`, so the nine sidebar lists it styles (History, Favorites, Downloads, the Alerts tree + its two sub-lists, Recordings, Queue, Recommended) stayed near-invisible-selected-text in Gruvbox exactly as the main list was before SEL-1. Calling `apply_list_selection(view)` as a SECOND registration (the naive fix) turned out to be worse than the bug: two `setStyleSheet` calls on one widget in the same theme-switch sweep make Qt's stylesheet engine discard the QPalette roles the first call just pinned — reproduced with a bare script, breaking on the very FIRST live switch, not just a later one. Fixed by extracting the shared body into a new public `theme.pin_list_selection(view, base) -> str` (pins the palette, appends the rule, strips a prior copy) that `apply_list_selection` now wraps in ONE `style_fn` registration and `make_seamless` calls from inside its OWN single registration — one `setStyleSheet` call per widget per switch either way. All nine consumers inherit through the one `make_seamless()` call, per the chokepoint rule. See F36 for a related, pre-existing bug this surfaced but does not fix. |
| F36 | **`apply_list_selection`'s QPalette pin only survives ONE theme switch, then sticks forever** | `gui/theme.py` `pin_list_selection()` (was `apply_list_selection`'s `_rebuild`) — affects every consumer: `main_window.py`'s `channels_list`/`series_tree`, `three_panel_section_nav.py`'s `section_list`, and now the nine `make_seamless()` sidebar lists | Found while proving F35's fix survives more than one switch: construct a view, switch theme once — `HighlightedText`/`Highlight` update correctly — switch AGAIN, and both roles stay frozen at the FIRST switch's values, in every palette, on the unmodified pre-F35 code too (reproduced on `apply_list_selection` alone, no `make_seamless` involved). Root cause is a Qt stylesheet-engine quirk, not this codebase's registry: once a widget has taken two or more DIFFERENT-content `setStyleSheet()` calls, Qt's CSS engine starts discarding QPalette roles a later `setPalette()` sets for anything the stylesheet text does not itself declare — reproduced in isolation with plain `QListWidget().setStyleSheet(...)` + `setPalette(...)` + a third differing `setStyleSheet(...)`. **Not fixed here**: it predates F35 (present in #715's merged code), is not scoped to sidebar/base.py, and the fix is a different mechanism entirely — likely re-deriving Highlight/HighlightedText at PAINT time via a custom item delegate rather than QPalette, or re-asserting the palette from a `QTimer.singleShot(0, …)` after Qt's own polish pass runs. Needs its own investigation and its own before/after proof across 2+ switches, not a one-line fix riding along with F35. |
| F37 | **`_get_all_collapsed`'s final row fetch does not inherit `get_all()`'s `raw_data` defer** | `core/repositories/channel.py` `_get_all_collapsed()` (~line 678) | Found while adding `include_raw` to `get_all()` (DB-3, #618 follow-up): the pre-existing comment on `get_all()` claimed "the collapsed path builds its subquery from THIS query, so deferring here covers both shapes" — true only for the WHERE-clause/grouping subquery. The final materialization (`self.session.query(ChannelDB).filter(ChannelDB.id.in_(rep_ids)).outerjoin(MetadataDB, ...)`) is a fresh, independent query that carries no `defer()` option, so a `collapse_variants=True` load still eagerly loads `raw_data` for its page of representative rows regardless of `include_raw`. Bounded to page size (not the whole corpus, unlike the pre-#618 bug), so not urgent, but the comment claiming coverage was corrected in the same edit that found the gap. Not fixed inline — it's a second query needing its own defer + its own N+1/DetachedInstanceError proof, out of this slice's scope. |
| F38 | **`ChannelRepository.count()` and `_query_channels`'s inline `has_adult` probe both still materialize `raw_data`, on EVERY channel-list load** | `core/repositories/channel.py` `count()` (~line 1142); `gui/main_window_channels.py` `_query_channels`'s `has_adult` line (~line 691) | Found writing `test_get_all_include_raw.py`'s "raw_data never appears in the emitted SQL" assertion against the REAL `_query_channels` caller path — it failed, not on `get_all()`'s own SELECT (which is clean), but on two OTHER statements. Both call bare `Query(ChannelDB).count()`, and SQLAlchemy's `.count()` wraps the FULL undeferred column list (raw_data included) as `SELECT count(*) FROM (SELECT channels.*, ... FROM channels WHERE ...)` — so SQLite still has to read every matching row's raw_data bytes off disk to evaluate the subquery, even though nothing in the result set uses them. Both run on **every single channel-list load** (`total = repos.channels.count(...)` and the adult-gate probe), right alongside the now-deferred `get_all()` call — so a real fraction of DB-3's measured win is still being paid on every load. Not fixed inline: out of DB-3's stated scope (`get_all()` only) and each needs its own before/after measurement. Mechanical fix: `.options(defer(ChannelDB.raw_data))` on both queries. |
| F39 | **Two reads of `global_filter_excluded_user_categories` in `discover_workers._LoaderWorker.run`, one paused-aware and one not** | `gui/discover_workers.py` — the new `excluded_user_categories` (via `filter_utils.global_exclusion_sets(self._config)[1]`, added to feed the shelf-query `excluded_categories` axis, #618) vs. the pre-existing `excluded_user_cats` (`getattr(self._config, "global_filter_excluded_user_categories", [])`, feeds `get_all_genres`... no — feeds `get_all_user_categories`'s category-LIST filter) | **Not a pure duplicate — a policy difference, found while wiring #618.** `global_exclusion_sets` empties every axis while Global Exclusions are paused (its documented contract); the categories-LIST read has no such gate, so pausing Global Exclusions today still hides a category from the Discover categories index while (after #618) unhiding its content from every OTHER shelf — a visible inconsistency between the two lists that predates this slice. Not fixed inline: unifying them means deciding whether the categories-INDEX should also respect pause, which is a product call on a surface this slice was not briefed to touch. |
|---|---|---|
...
| D32 | The "is this event still on" window, three times | `channel_stats.LIVE_WINDOW`, `relative_time.ELAPSED_WINDOW_S`, `signal_check_manager.LIVE_WINDOW` — each its own `timedelta(hours=4)` / `4 * _HOUR`, each with a comment saying no provider sends an end time | **PARTLY DONE.** Two of the three now read `event_datetime.DEFAULT_EVENT_DURATION`, and the surfaces prefer the provider's real `event_stop_time` where it exists. `signal_check_manager`'s copy is deliberately left: it answers a DIFFERENT question — which streams are worth PROBING — and narrowing it to real windows would change probe volume, which is a measurement, not a rename. Not a pure duplicate: the shared constant is the fallback, the third is a policy about background work. **Amended (audit slice 4, 2026-09-07):** `relative_time.ELAPSED_WINDOW_S` had zero readers anywhere (its only consumer, `is_countdown_live`, was itself uncalled) — deleted along with the rest of that file's dead cluster. Two copies remain: `channel_stats.LIVE_WINDOW` and `signal_check_manager.LIVE_WINDOW` |
...
| D35 | Two category→region readers with different vocabularies | `channel_name_utils.split_category_prefix()` (PR #730: feeds `detected_prefix`/`detected_region` for exclusions, validates against `REGION_FULL_NAMES`) vs the pre-existing `region_code_from_category()` (feeds the tag/facet system, validates against `CODE_FACETS`) | A POLICY difference, not a pure duplicate — and load-bearing: `CODE_FACETS` deliberately classes AR as language-only (Arabic, not Argentina — the #138 fix), so the facet reader must NOT treat AR as a region, while the exclusion reader MUST (the user's "exclude AR" means the Arabic-package content). Unifying them on one vocabulary would re-break one side or the other. Logged so nobody "deduplicates" this into a regression; if a third reader ever appears, THAT is the moment to build the two-vocabulary chokepoint |
| D34 | The sports keyword map, twice | `metatv/data/sports_definitions.yaml` (the real source, user-overridable) vs `special_content._DEFAULT_SPORT_KEYWORDS`/`_DEFAULT_LEAGUE_KEYWORDS` (in-memory fallback used only when the bundled YAML fails to read) | A policy difference, not a pure duplicate: the fallback must exist (a broken YAML read should not empty the Sports view), but it is a hand-mirror kept in sync by a comment saying "a keyword added to one must be added to the other" (added with the FloSports slice, PR #725, which edited both in lockstep). A prose sync rule is what drifts; the mechanical fix is a test that loads the bundled YAML and asserts the fallback dicts equal it — then the mirror cannot go stale and the comment becomes enforcement. Small standalone slice |
| D37 | Two owners of sidebar row visibility (row budget vs section filters/folds) | `row_budget.py`'s `apply_row_budget`/`apply_tree_row_budget` unconditionally un-hid every row/child on every resize and populate, while `queue.py`'s find-in-queue filter (`_apply_filter`) and `alerts_epg.py`'s fold (`_apply_upcoming_collapse`) hide rows on purpose | **DONE (HIDE-1).** Not a live duplicate of intent — a leftover of the removed "Show N more" mode (2026-09-02): the budget stopped hiding anything that day, so the loop had nothing left to undo and just fought whatever else hid a row. `alerts_epg`'s top-level group fold was unaffected (the loop only ever touched children of a VISIBLE group), but `queue.py`'s filter was resurrected by a mere splitter drag. Both un-hide loops deleted; the budget now only sets the scrollbar policy and calls `fit_to_rows`, which already measures visible rows. Visibility has exactly one owner per row now — whichever filter or fold hid it |
| D38 | `_main_repo`/`resolve_py`, hand-derived per script | `_main_repo` was defined in `verify_pr.sh`, `merge_pr.sh` and `open_batch.sh`; `resolve_py` only in `verify_pr.sh`, so `.githooks/pre-push` and `scripts/pytest_verdict.sh` each hardcoded `PY="venv/bin/python"` relative to CWD instead — exit 127 from a git worktree with no venv symlink, which cost two blocked pushes on 2026-09-03 (GATE-7) | **DONE** for the five files this slice touched. `scripts/repo_python.sh` is now the single sourced copy (moved verbatim out of `verify_pr.sh`, which had the correct answer first); `verify_pr.sh`, `merge_pr.sh`, `open_batch.sh`, `pytest_verdict.sh` and `.githooks/pre-push` all source it. Pure duplicate, no policy difference. **Now fully closed (GATE-7b)**: `run.sh`'s byte-identical `_main_repo`/`resolve_py` pair is deleted in favour of `source scripts/repo_python.sh`; `scripts/ci_shards_local.sh` and `scripts/ship_batch.sh` (the latter already a known legacy exception elsewhere, for its own separate bare-pytest reason) now source the library too instead of resolving `venv/bin/python` by hand. `tests/test_local_gates_have_one_path.py::test_scripts_resolve_python_through_one_path`'s shrink-only known set is empty again |

| D39 | Two hand-listed `closeEvent` skeleton builders | `tests/test_close_event_cleanup.py::_build_mock_window` and `tests/test_shutdown_safety.py::_build_close_event_window` (whose docstring says it is "wired the same way as" the first) each enumerate the managers `closeEvent` touches | Pure duplicate — and the enumeration trap CLAUDE.md names: REC-3's quit guard (`recording_manager.progress()`) broke BOTH in CI, and both had to be patched by hand. The fix is one `tests/conftest.py::make_close_event_window()` that both files call, so the next attribute `closeEvent` reads is added once. Not done in #741 (a feature PR) — its own small slice |

| D40 | A transfer row's `meta_parts` never rendered at all | `gui/sidebar/transfer_rows.add_transfer_row()` never passed `density=` to `build_chip_row`, so its own default (`DENSITY_COMPACT`) always won — `comfortable = density == DENSITY_COMFORTABLE and bool(meta)` was False for every download/recording row ever built, whatever `meta_parts` held. `gui/sidebar/downloads.py` and `gui/sidebar/recordings.py` both call it this way | **DONE for Downloads** (the queue-with-reasons/speed/ETA slice, 2026-09-05): `_add_download_row` now passes `density=DENSITY_COMFORTABLE` explicitly — deliberately NOT the user's general `sidebar_row_density` preference, since the second line here (why a row is stuck, its speed, its ETA) is the reason the section exists, not a decorative extra; `build_chip_row` already collapses back to one line by itself whenever `meta` is empty. Also had to extend `build_chip_row`'s comfortable branch to place `tail_widget` (the progress bar) on the title line — previously ONLY the compact branch rendered `tail_widget` at all, so a comfortable row could never have shown one; additive-only, since no comfortable-density call site had ever passed `tail_widget` before. **Recordings' copy is untouched** — its meta line (channel name / start time / recorded bytes) still silently renders nothing. Same one-line fix would apply there; left alone because it is outside this slice's brief, and because Recordings' own row (no reason/rate/ETA to show, just three facts already visible via chips/tail today) makes the value of switching it to comfortable less clear-cut without its own design pass |
| D41 | Two provider-name-for-a-notification lookups | `main_window_downloads.record_channel_window` (REC Option B, landed independently) inlines `provider_db = repos.providers.get_by_id(channel.provider_id); name = provider_db.name if provider_db else "This source"`; `main_window_downloads._recording_source_name` (REC-2, the persistent recording notice) is the same query as a method, falling back to `"this source"` (lowercase) | Found merging two independent PRs that touched the same file the same week. Not folded into one in either slice: `tests/test_record_window.py::test_record_channel_window_schedules_with_the_dialogs_own_padding` (or its neighbour) asserts the literal capitalized `"This source"` string, so changing `record_channel_window` to call `_recording_source_name` would flip that fallback's case and fail an already-integrated test outside this PR's brief. A real duplicate, not a policy difference (both want "the provider's name, or a generic fallback") — but reconciling the capitalization is a one-line decision for whoever owns both call sites at once, not a rider on either feature |
| D42 | The dead-signal-streak visibility axis (VE-1) reaches four of the five scoped surfaces | `channel_visibility.VisibilityScope.dead_signal_streak_floor` is applied by `ChannelRepository.get_all` (list + search), `discovery_engine`'s eleven shelf functions (Discover, through `discover_workers`), `preference_engine.score_candidates` (Recommendations) and `visibility_resolver.resolve_scope` (Similar / lens); **`tag.py`'s facet-value-count and saved-recipe sampler family does not build a `VisibilityScope` with it** — those queries thread `excluded_provider_ids`/keyword/prefix axes by hand (`sample_channels_by_tag_facets`, the Browse counts), so a dead live channel still counts in a tag facet and can be sampled into a recipe shelf. Policy difference, not a pure duplicate: the counts family was migrated onto `VisibilityScope` for the provider axis only. Fix = pass the floor into the same block that gates on `excluded_provider_ids is not None` (see the comment at `discover_workers.py` ~200 for why that block is gated). |
