...
| D31 | `channel_list_delegate.py` is at the 1000-line floor and every row kind pushes it over | The section band, the person sub-heading and the channel row all paint from one file | **OPEN, and now overdue** — the fixture state mark (#533) added six more lines and needed a second rebaseline in one day, which is the ratchet doing its job: it is pointing at a file that cannot absorb another row kind. The next row feature should be BLOCKED on this split. Same shape D26 was for the model: the ratchet is pointing at a second subject inside the first. `channel_list_section_band.py` took the band out (219 lines); the natural next lift is the row's chip/badge painting, which is cohesive and has no dependency on the label paths. A slice of its own — surgery on the paint hot path does not ride along with a feature |
